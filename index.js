...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// External is in the global scope, where as internal is in the block scope of the MyFunction function. 

// This means that we could console log the external variable anywhere but the internal variable *must* be accessed within the myFunction function.
...
function summation(number) {
  ...
  console.log(counter)
  let sum = counter.reduceRight(myFunction);
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const displayNames = [];
    arr.forEach((object) => {
      displayNames.push(`name: ${object.animal_name}, scientific: ${object.scientific_name}`)
    })
    return displayNames;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const animalLowPopCount = arr.filter(myFunction)

    function myFunction(value, index, array){
      return value.population < 5;
    }

    return animalLowPopCount;
  }
  ...
  */
  ...
  function USApop(arr){
  let sum = arr.reduce((total, animal) => {
